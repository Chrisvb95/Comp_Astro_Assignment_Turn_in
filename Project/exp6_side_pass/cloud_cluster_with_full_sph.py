...
def plot_cloud_cluster(cluster, sph, stars, title, vrange=[-5,3]):
    ...
    cbar.set_label(r'log($\rho$) [amu/cm$^3$]', fontsize=15, rotation=270, labelpad=25)

    ax1.set_title(title+' Myr', weight='bold')
    ax2.set_title('Projected density map of the cloud', fontsize=15, pad=10)
...
def cluster_init(N, Mtot, Rvir, init_v, init_position, alpha=-2.35, seed=1):
    ...
    # new_powerlaw_mass_distribution(N, Mmin, Mmax, alpha)
    mZAMS = new_powerlaw_mass_distribution(N, 0.6|units.MSun, 160.|units.MSun, alpha=alpha)
    cluster.mass = mZAMS
...
def resolve_sinks(sph, stars, cloud, density_thr, model_time):
    ...
    # if there is any gas particles that has been selected out, consider a star formation event
        ...
        with open(printout_file,'a') as pf:
            pf.write('confirmed %d new stars: '%len(candidate_stars))
...
def evolve(cluster,cloud, converter_grav,converter_sph, t_end, dt_sph, dt_diag,\
    ...
    for i,t in enumerate(times):
        ...
        dt_sph = np.round((shortest_ts/10.).value_in(unit_time), 2) | unit_time
        with open(printout_file, 'a') as pf:
...
if __name__ == '__main__':
    ...
    v_cluster = (3.20462,2.78036,0) | units.km/units.s   # b = 20pc
    ...
    cluster, conv_grav = cluster_init(N_cluster, Mtot_cluster, Rvir_cluster,\
                                              v_cluster, p_cluster, alpha=-1.5, seed=seed)
